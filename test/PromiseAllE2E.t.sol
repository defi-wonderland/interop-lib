...
contract PromiseAllE2ETest is Test {
    ...
    function test_promiseAllWithHarnessAutomation() public {
        ...
        bytes32 finalCallback =
            callbackContract.then(promiseAllId, address(target), target.handlePromiseAllResult.selector);
        ...
        assertEq(
            ...
            "Timeout1 should be resolved"
        );
        assertEq(
            uint256(promiseContract.status(timeout2)),
            uint256(Promise.PromiseStatus.Resolved),
            "Timeout2 should be resolved"
        );
        assertEq(
            uint256(promiseContract.status(promiseAllId)),
            uint256(Promise.PromiseStatus.Resolved),
            "PromiseAll should be resolved"
        );
        assertEq(
            uint256(promiseContract.status(finalCallback)),
            uint256(Promise.PromiseStatus.Resolved),
            "Final callback should be resolved"
        );
    ...
    function test_promiseAllWithFailureAndHarness() public {
        ...
        bytes32 errorCallback =
            callbackContract.catchError(promiseAllId, address(errorTarget), errorTarget.handleError.selector);
    ...
    function test_complexPromiseOrchestration() public {
        ...
        vm.prank(alice);
        bytes32 finalCallback =
            callbackContract.then(promiseAllId, address(aggregator), aggregator.aggregateResults.selector);
        ...
        assertEq(
            uint256(promiseContract.status(timeoutPromise)),
            uint256(Promise.PromiseStatus.Pending),
            "Timeout should be pending"
        );
        assertEq(
            uint256(promiseContract.status(callback1)),
            uint256(Promise.PromiseStatus.Pending),
            "Callback1 should be pending"
        );
        assertEq(
            uint256(promiseContract.status(callback2)),
            uint256(Promise.PromiseStatus.Pending),
            "Callback2 should be pending"
        );
        assertEq(
            uint256(promiseContract.status(promiseAllId)),
            uint256(Promise.PromiseStatus.Pending),
            "PromiseAll should be pending"
        );
        assertEq(
            uint256(promiseContract.status(finalCallback)),
            uint256(Promise.PromiseStatus.Pending),
            "Final callback should be pending"
        );
        ...
        assertEq(
            uint256(promiseContract.status(callback1)),
            uint256(Promise.PromiseStatus.Resolved),
            "Callback1 should be resolved"
        );
        assertEq(
            uint256(promiseContract.status(callback2)),
            uint256(Promise.PromiseStatus.Resolved),
            "Callback2 should be resolved"
        );
        ...
        assertEq(
            ...
            "aggregated-processed-timeout-processed-timeout",
            "Final result should be aggregated"
        );
