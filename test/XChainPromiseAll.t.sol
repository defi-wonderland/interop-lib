...
contract XChainPromiseAllTest is Test, Relayer {
    ...
    function test_CrossChainPromiseAggregation() public {
        ...
        assertEq(
            uint8(promiseA.status(promiseAllId)), uint8(Promise.PromiseStatus.Resolved), "PromiseAll should be resolved"
        );
    ...
    function test_PromiseAllResultSharing() public {
        ...
        assertEq(
            uint8(promiseB.status(promiseAllId)),
            uint8(Promise.PromiseStatus.Resolved),
            "PromiseAll should be resolved on Chain B"
        );
    ...
    function test_CrossChainPromiseAllWithCallback() public {
        ...
        vm.selectFork(forkIds[0]);
        uint256 chainBId = chainIdByForkId[forkIds[1]];
        bytes32 callbackPromise =
            callbackA.thenOn(chainBId, promiseAllId, address(target), target.handlePromiseAll.selector);
