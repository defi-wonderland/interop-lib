...
import {PromiseCallback3} from "src/PromiseCallback3.sol";
import {CrosschainSwapper3} from "src/CrosschainSwapper3.sol";
...
/// @title CrosschainSwapper3E2E
/// @notice E2E test demonstrating CrosschainSwapper3 cross-chain swap workflow
/// @dev Flow: Chain A initSwap -> Chain B relaySwap -> Chain B bridgeBack -> Chain A
contract CrosschainSwapper3E2ETest is Test, Relayer {
    // Promise system contracts (deployed on both chains)
    PromiseCallback3 public promiseCallbackA;
    PromiseCallback3 public promiseCallbackB;

    // Validator contracts
    Validator public validatorA;
    Validator public validatorB;

    // CrosschainSwapper3 contracts
    CrosschainSwapper3 public crosschainSwapperA;
    CrosschainSwapper3 public crosschainSwapperB;
    ...
    function setUp() public {
        ...
        // Deploy PromiseCallback3 contracts using CREATE2 for same addresses
        vm.selectFork(forkIds[0]);
        promiseCallbackA = new PromiseCallback3{salt: bytes32(0)}();

        vm.selectFork(forkIds[1]);
        promiseCallbackB = new PromiseCallback3{salt: bytes32(0)}();

        // Verify same addresses
        require(
            address(promiseCallbackA) == address(promiseCallbackB), "PromiseCallback3 contracts must have same address"
        );
        ...
        // Deploy CrosschainSwapper3 contracts
        vm.selectFork(forkIds[0]);
        crosschainSwapperA =
            new CrosschainSwapper3{salt: bytes32(0)}(address(promiseCallbackA), address(routerA), address(validatorA));

        vm.selectFork(forkIds[1]);
        crosschainSwapperB =
            new CrosschainSwapper3{salt: bytes32(0)}(address(promiseCallbackB), address(routerB), address(validatorB));

        // Verify same addresses
        require(
            address(crosschainSwapperA) == address(crosschainSwapperB),
            "CrosschainSwapper3 contracts must have same address"
        );
    ...
    /// @notice Test successful cross-chain swap using CrosschainSwapper3
    /// @dev Flow: Chain A initSwap -> Chain B relaySwap -> Chain B bridgeBack -> Chain A
    function test_CrosschainSwapper3_CrossChainSwap_Success() public {
        console.log("=== Testing CrosschainSwapper3 Cross-Chain Swap Success ===");
        ...
        // Approve tokens for CrosschainSwapper3
        ...
        PromiseCallback3.Promise memory bridgePromise = promiseCallbackB.getPromise(bridgeId);
        console.log("Bridge promise status:", uint256(bridgePromise.status));
        console.log("CrosschainSwapper3 tokenOut balance:", tokenOut.balanceOf(address(crosschainSwapperB)));
        console.log("");
        ...
        PromiseCallback3.Promise memory bridgeBackPromise = promiseCallbackB.getPromise(bridgeBackId);
    ...
    function test_CrosschainSwapper3_CrossChainSwap_Failure() public {
        console.log("=== Testing CrosschainSwapper3 Cross-Chain Swap Failure ===");
        ...
        PromiseCallback3.Promise memory errorPromise = promiseCallbackB.getPromise(bridgeBackOnErrorId);
