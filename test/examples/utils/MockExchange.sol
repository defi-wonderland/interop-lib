...
contract MockExchange {
    ...
    event SwapExecuted(
        address indexed tokenIn, address indexed tokenOut, uint256 amountIn, uint256 amountOut, address indexed user
    );

    /// @notice Event emitted when a swap fails
    event SwapFailed(
        address indexed tokenIn, address indexed tokenOut, uint256 amountIn, address indexed user, string reason
    );
