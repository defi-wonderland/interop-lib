...
contract CrossChainSwapExampleTest is Test, Relayer {
    ...
    function test_CrossChainSwap_FailureAndRollback() public {
        ...
        console.log("SETUP: Executing bridge operation");
        (bytes32 bridgePromiseId, bytes32 bridgeCallbackId) =
            bridgeA.bridgeTokens(address(token2), token2Amount, chainIdByForkId[forkIds[1]], user);

        console.log("SETUP: Chaining final swap (will detect failure and trigger rollback)");
        bytes32 finalSwapCallbackId = callbackA.thenOn(
            chainIdByForkId[forkIds[1]], bridgePromiseId, address(this), this.executeFinalSwapWithBranching.selector
        );

        console.log("SETUP: Chaining automatic bridge-back for final swap failures");
        bytes32 bridgeBackCallbackId = callbackA.catchErrorOn(
            chainIdByForkId[forkIds[1]], finalSwapCallbackId, address(this), this.bridgeTokensBack.selector
        );
        ...
        console.log(
            "Flow: Token1 -> Token2 -> Bridge -> Token2 (Chain B) -> [SWAP FAILS] -> Bridge Back -> Token2 (Chain A)"
        );
    }
    ...
    function test_PromiseBasedAutomaticRollback() public {
        ...
        bytes32 localRollbackCallbackId =
            callbackA.catchError(workflowPromiseId, address(this), this.executeAutomaticRollback.selector);
