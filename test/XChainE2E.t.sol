...
contract XChainE2ETest is Test, Relayer {
    ...
    function test_PeriodicFeeCollectionAndBurning() public {
        ...
        assertEq(
            uint8(promiseA.status(nextTimeoutId)),
            uint8(Promise.PromiseStatus.Pending),
            "Next timeout should be pending"
        );
    }
    ...
    function test_FeeCollectionFailureHandling() public {
        ...
        assertEq(
            uint8(promiseA.status(chainAPromise)),
            uint8(Promise.PromiseStatus.Rejected),
            "Chain A fee collection should be rejected"
        );
        ...
        assertEq(
            uint8(promiseA.status(promiseAllId)), uint8(Promise.PromiseStatus.Rejected), "PromiseAll should be rejected"
        );
        ...
        if (callbackA.canResolve(burnCallbackId)) {
            ...
            callbackA.resolve(burnCallbackId);
            assertEq(
                uint8(promiseA.status(burnCallbackId)),
                uint8(Promise.PromiseStatus.Rejected),
                "Burn callback should be rejected when PromiseAll fails"
            );
        }
    ...
    function test_RemotePromiseTimeoutOrchestration() public {
        ...
        assertFalse(
            callbackA.exists(chainBFeeCallback), "Cross-chain callback should not exist on Chain A after transfer"
        );
        ...
        bytes32 burnCallback = callbackA.then(promiseAllId, address(feeBurner), FeeBurner.burnFees.selector);
        ...
        assertEq(
            uint8(promiseA.status(remoteTimeoutId)),
            uint8(Promise.PromiseStatus.Resolved),
            "Remote timeout should be resolved"
        );
...
contract CronScheduler {
    ...
    function startPeriodicFeeCollection(
        ...
        bytes32 executionCallbackId =
            callbackContract.then(firstTimeoutId, address(this), CronScheduler.executeCycleCallback.selector);
    ...
    function executeCycle(uint256 cycleId) external {
        ...
        bytes32 chainAFeePromise =
            callbackContract.then(nextTimeoutIds[cycleId], cycle.chainAFeeCollector, FeeCollector.collectFees.selector);

        bytes32 chainBFeePromise = callbackContract.thenOn(
            cycle.chainBId, nextTimeoutIds[cycleId], cycle.chainBFeeCollector, FeeCollector.collectFees.selector
        );
        ...
        bytes32 burnCallbackId = callbackContract.then(promiseAllId, cycle.feeBurner, FeeBurner.burnFees.selector);
        ...
        bytes32 nextExecutionCallbackId =
            callbackContract.then(nextTimeoutId, address(this), CronScheduler.executeCycleCallback.selector);
    ...
    function executeCycleCallback(bytes memory /* data */ ) external returns (string memory) {
        ...
        for (uint256 cycleId = 1; cycleId < nextCycleId; cycleId++) {
            ...
            if (
                timeoutId > 0 && promiseContract.status(timeoutId) == Promise.PromiseStatus.Resolved
                    && (
