...
contract XChainSetTimeoutTest is Test, Relayer {
    ...
    function test_CrossChainTimeoutWithCallback() public {
        ...
        bytes32 callbackPromise =
            callbackA.thenOn(chainBId, timeoutPromise, address(target), target.handleTimeout.selector);
        ...
        assertEq(
            uint8(promiseA.status(timeoutPromise)), uint8(Promise.PromiseStatus.Resolved), "Timeout should be resolved"
        );
        ...
        assertEq(
            uint8(promiseB.status(callbackPromise)),
            uint8(Promise.PromiseStatus.Resolved),
            "Callback should be resolved on Chain B"
        );
    }
    ...
    function test_CrossChainTimeoutResolution() public {
        ...
        vm.selectFork(forkIds[1]);
        assertEq(
            uint8(promiseB.status(timeoutPromise)),
            uint8(Promise.PromiseStatus.Resolved),
