...
contract XChainCallbackTest is Test, Relayer {
    ...
    function test_CrossChainThenCallback() public {
        ...
        bytes32 callbackPromiseId =
            callbackA.thenOn(chainBId, parentPromiseId, address(target), target.handleSuccess.selector);
        ...
        assertEq(
            uint8(promiseB.status(parentPromiseId)), uint8(Promise.PromiseStatus.Resolved), "Parent should be resolved"
        );
        ...
        assertEq(
            uint8(promiseB.status(callbackPromiseId)),
            uint8(Promise.PromiseStatus.Resolved),
            "Callback promise should be resolved"
        );
    ...
    function test_CrossChainCatchCallback() public {
        ...
        uint256 chainBId = chainIdByForkId[forkIds[1]];
        bytes32 callbackPromiseId =
            callbackA.catchErrorOn(chainBId, parentPromiseId, address(target), target.handleError.selector);
        ...
        assertTrue(promiseB.exists(parentPromiseId), "Parent promise should exist on Chain B");
        assertEq(
            uint8(promiseB.status(parentPromiseId)), uint8(Promise.PromiseStatus.Rejected), "Parent should be rejected"
        );
    ...
    function test_CrossChainThenCallbackNotExecutedWhenParentRejected() public {
        ...
        assertEq(
            uint8(promiseB.status(callbackPromiseId)),
            uint8(Promise.PromiseStatus.Rejected),
            "Callback promise should be rejected"
        );
    }
    ...
    function test_MultipleCrossChainCallbacks() public {
        ...
        uint256 chainBId = chainIdByForkId[forkIds[1]];
        bytes32 callback1 =
            callbackA.thenOn(chainBId, parentPromiseId, address(target1), target1.handleSuccess.selector);
        bytes32 callback2 =
            callbackA.thenOn(chainBId, parentPromiseId, address(target2), target2.handleSuccess.selector);
    ...
    function test_CallbackForRemotePromise() public {
        ...
        bytes32 callbackPromiseId = callbackB.then(remotePromiseId, address(target), target.handleSuccess.selector);
        ...
        assertEq(
            uint8(promiseB.status(remotePromiseId)), uint8(Promise.PromiseStatus.Resolved), "Parent should be resolved"
        );
    ...
    function test_CatchCallbackForRemotePromise() public {
        ...
        bytes32 callbackPromiseId = callbackB.catchError(remotePromiseId, address(target), target.handleError.selector);
        ...
        assertTrue(promiseB.exists(remotePromiseId), "Parent promise should now exist on Chain B");
        assertEq(
            uint8(promiseB.status(remotePromiseId)), uint8(Promise.PromiseStatus.Rejected), "Parent should be rejected"
        );
        assertTrue(callbackB.canResolve(callbackPromiseId), "Callback should be resolvable now");
    ...
    function test_MultipleCallbacksForSameRemotePromise() public {
        ...
        bytes32 errorCallback =
            callbackB.catchError(remotePromiseId, address(errorTarget), errorTarget.handleError.selector);
        ...
        assertEq(
            uint8(promiseB.status(errorCallback)),
            uint8(Promise.PromiseStatus.Rejected),
            "Error callback should be rejected"
        );
    }
    ...
    function test_CrossChainCallbackForRemotePromise() public {
        ...
        uint256 chainBId = chainIdByForkId[forkIds[1]];
        bytes32 callbackPromiseId =
            callbackA.thenOn(chainBId, remotePromiseId, address(target), target.handleSuccess.selector);
    ...
    function test_LocalCallbackAuthTracking() public {
        ...
        vm.prank(alice);
        bytes32 thenCallbackId = callbackA.then(parentPromiseId, address(target), target.handleSuccess.selector);

        // Register catch callback as bob
        address bob = address(0x2);
        vm.prank(bob);
        bytes32 catchCallbackId = callbackA.catchError(parentPromiseId, address(target), target.handleError.selector);
    ...
    function test_AuthTrackingWithMultipleUsers() public {
        ...
        vm.selectFork(forkIds[0]);
        vm.prank(alice);
        bytes32 aliceLocalCallback = callbackA.then(promiseA1, address(target), target.handleSuccess.selector);

        // Bob registers cross-chain callback from Chain A to Chain B
        vm.prank(bob);
        bytes32 bobXChainCallback =
            callbackA.catchErrorOn(chainBId, promiseA2, address(target), target.handleError.selector);

        // Charlie registers local callback on Chain B
        vm.selectFork(forkIds[1]);
        vm.prank(charlie);
        bytes32 charlieLocalCallback = callbackB.then(promiseB1, address(target), target.handleSuccess.selector);
