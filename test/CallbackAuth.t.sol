...
contract CallbackAuthTest is Test {
    ...
    function test_callbackContextAvailableDuringExecution() public {
        ...
        bytes32 callbackPromiseId =
            callbackContract.then(parentPromiseId, address(contextTarget), contextTarget.handleWithContext.selector);
        ...
        assertEq(
            contextTarget.lastSourceChain(),
            block.chainid,
            "Target should know callback was registered on current chain"
        );
    ...
    function test_authorizedCallbackSucceeds() public {
        ...
        bytes32 callbackId = callbackContract.then(promiseId, address(vault), vault.performPrivilegedOperation.selector);
    ...
    function test_publicCallbackWorksForEveryone() public {
        ...
        bytes32 callbackId = callbackContract.then(promiseId, address(vault), vault.performPublicOperation.selector);
    ...
    function test_crossChainAuthenticationPreserved() public {
        ...
        vm.prank(trustedUser);
        bytes32 callbackId =
            callbackContract.then(promiseId, address(vault), vault.performChainSpecificOperation.selector);
    ...
    function test_reentrancyProtectionWorks() public {
        ...
        vm.prank(alice);
        bytes32 callbackId =
            callbackContract.then(parentPromiseId, address(maliciousTarget), maliciousTarget.maliciousCallback.selector);
        ...
        Promise.PromiseStatus status = promiseContract.status(callbackId);
        assertEq(
            uint256(status), uint256(Promise.PromiseStatus.Rejected), "Callback should be rejected due to re-entrancy"
        );
        ...
        if (actualError.length >= expectedError.length) {
            for (uint256 i = 0; i <= actualError.length - expectedError.length; i++) {
                bool matches = true;
                for (uint256 j = 0; j < expectedError.length; j++) {
    ...
    function test_normalOperationUnaffected() public {
        ...
        vm.prank(alice);
        bytes32 callbackId =
            callbackContract.then(parentPromiseId, address(maliciousTarget), maliciousTarget.normalCallback.selector);
