...
contract CallbackTest is Test {
    ...
    event CallbackRegistered(
        bytes32 indexed callbackPromiseId, bytes32 indexed parentPromiseId, Callback.CallbackType callbackType
    );
    ...
    function test_createThenCallback() public {
        ...
        bytes32 callbackPromiseId =
            callbackContract.then(parentPromiseId, address(testTarget), testTarget.handleSuccess.selector);
        ...
        assertEq(
            uint256(callbackData.callbackType), uint256(Callback.CallbackType.Then), "Callback type should be Then"
        );
    ...
    function test_createCatchCallback() public {
        ...
        bytes32 callbackPromiseId =
            callbackContract.catchError(parentPromiseId, address(testTarget), testTarget.handleError.selector);

        assertEq(callbackPromiseId, expectedCallbackId, "Callback promise ID should match expected global ID");

        Callback.CallbackData memory callbackData = callbackContract.getCallback(callbackPromiseId);
        assertEq(
            uint256(callbackData.callbackType), uint256(Callback.CallbackType.Catch), "Callback type should be Catch"
        );
    ...
    function test_canCreateCallbackForNonExistentPromise() public {
        ...
        bytes32 callback1 =
            callbackContract.then(bytes32(uint256(999)), address(testTarget), testTarget.handleSuccess.selector);
        bytes32 callback2 =
            callbackContract.catchError(bytes32(uint256(999)), address(testTarget), testTarget.handleError.selector);
        ...
        assertFalse(
            callbackContract.canResolve(callback1), "Then callback should not be resolvable for non-existent promise"
        );
        assertFalse(
            callbackContract.canResolve(callback2), "Catch callback should not be resolvable for non-existent promise"
        );
    ...
    function test_resolveThenCallback() public {
        ...
        assertEq(
            uint256(promiseContract.status(callbackPromiseId)),
    ...
    function test_callbackHandlesTargetFailure() public {
        ...
        vm.prank(bob);
        bytes32 callbackPromiseId =
            callbackContract.then(parentPromiseId, address(testTarget), testTarget.alwaysFails.selector);
        ...
        assertEq(
            uint256(promiseContract.status(callbackPromiseId)),
            uint256(Promise.PromiseStatus.Rejected),
            "Callback promise should be rejected when target fails"
        );
    }
    ...
    function test_multipleCallbacksOnSamePromise() public {
        ...
        vm.prank(bob);
        bytes32 callback2 =
            callbackContract.then(parentPromiseId, address(testTarget2), testTarget2.handleSuccess.selector);
        ...
        assertEq(
            ...
            uint256(Promise.PromiseStatus.Resolved),
            "First callback should be resolved"
        );
        assertEq(
            uint256(promiseContract.status(callback2)),
            uint256(Promise.PromiseStatus.Resolved),
            "Second callback should be resolved"
        );
    ...
    function test_authTrackingForLocalCallbacks() public {
        ...
        vm.prank(bob);
        bytes32 thenCallbackId =
            callbackContract.then(parentPromiseId, address(testTarget), testTarget.handleSuccess.selector);

        // Register catch callback as alice
        vm.prank(alice);
        bytes32 catchCallbackId =
            callbackContract.catchError(parentPromiseId, address(testTarget), testTarget.handleError.selector);
